criaCartao(
    'Matemática',
    'Qual a fórmula de Bhaskara?',
    'x=(-b±√b²-4.a.b)/2.a')
    
    criaCartao(
    'Matemática',
    'Qual é o teorema de Pitágoras?',
    'a²=b²+c²')
    
    criaCartao(
    'Matemática',
    'Como calcular o termo geral de uma PA?',
    'an=a¹+(n-1).r')
    
    criaCartao(
    'Matemática',
    'Como calcular a soma dos termos de uma PA?',
    'Sn=(n.(a¹+an))/2')
    
    criaCartao(
    'Matemática',
    'Como calcular o termo geral de uma PG?',
    'an=a1.qn-¹')
    
    criaCartao(
    'Matemática',
    'Como calcular a soma dos termos de uma PG Finita?',
    'Sn=(a1.(Qn-1))/Q-1')
    
    criaCartao(
    'Física',
    'Qual a fórmula da velocidade',
    't/m')
    
    criaCartao(
    'Física',
    'Qual a fórmula da enegia',
    'E=mc²')
    
    criaCartao(
    'Física',
    'Qual a medida correspondente a aceleração da gravidade da Terra',
    '9,8m/s')

    criaCartao(
    'Física',
    'Qual a primeira lei de Newton',
    'Princípio da Inércia')
    
    criaCartao(
    'Física',
    'Qual a segunda lei de Newton',
    'Lei da superposição de forças')
    
    criaCartao(
    'Física',
    'Qual a terceira lei de Newton',
    'Princípio da ação e reação')
    
    criaCartao(
    'História',
    'Quem foi o primeiro imperador de Roma',
    'Rómulo Augustolo')
    
    criaCartao(
    'História',
    'Qual o primeiro imperador de Roma de que temos registro',
    'Augusto Cesar')
    
    criaCartao(
    'História',
    'Quando morreu Getúlio Vargas',
    '24 de agosto de 1954)')
    
    criaCartao(
    'História',
    'Quando foi a Proclamação da República',
    '15 de novembro de 1889')
    
    criaCartao(
    'Química',
    'Como identificar um ácido',
    'Presença de hidrogênio pressupondo outro elemento ')
    
    criaCartao(
    'Química',
    'Como identificar uma base',
    'Presença de hidroxila OH')
    
    criaCartao(
    'Química',
    'Quantas ligações faz o carbono',
    '6')
    
    criaCartao(
    'Química',
    'Quantas ligações faz o hidrogênio',
    '1')